...
class ADSurface : public ADIntegratedBC

{
public:
  static InputParameters validParams();
  
  ADSurface(const InputParameters & parameters);
...
protected:
  ...
  const Real _eta1;  /** Pre-exponential factor */
  const Real _eta2;  /** Pre-exponential factor */
  const Real _dHdd1; /** dH**1 */
  const Real _dHdd2; /** dH**2 */
  const Real _dS1;   /** Entropy1 */
  const Real _dS2;   /** Entropy2 */
  const Real _dH1;   /** Enthalpy1 */
