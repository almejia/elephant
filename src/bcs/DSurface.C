...
registerMooseObject("MooseApp", DSurface);

template <>
InputParameters
validParams<DSurface>()
{
...
DSurface::DSurface(const InputParameters & parameters)
  : NodalBC(parameters),
...
DSurface::computeQpResidual()
{
  ...
  return ( _eta1 * exp(-_dHdd1/(_R*_T))) * (_Pco2*(1-_vac_var[_qp]-_u[_qp]) - (_u[_qp]/ (exp(_dS1/_R)*exp(-_dH1/(_R*_T))) )) - (_eta2 * exp(-_dHdd2/(_R*_T))) * (_u[_qp]*_vac_var[_qp]*_ele_var[_qp]*_ele_var[_qp] - (_Pco * (1-_vac_var[_qp]-_u[_qp]) * (1-_vac_var[_qp]-_u[_qp]) * (1-_ele_var[_qp]) * (1-_ele_var[_qp]) /(exp(_dS2/_R)*exp(-_dH2/(_R*_T))) ) )  ;
}
