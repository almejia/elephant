...
validParams<Vacancy>()
{
  ...
  params.addParam<Real>("nv", 8.2975e4,"Cation site density (ny). Default is 41488 = 4/(avnum*latpar^3)." );
  params.addParam<Real>("nvv", 2.4893e5, "Anion-anion next nearest-neighbor bond density (nvv). Default is 165952 = 4/(avnum*latpar^3).");
...
Vacancy::Vacancy(const InputParameters & parameters) : Kernel(parameters),
...
{
  return -_cv * _grad_u[_qp] * _grad_test[_i][_qp] + ( (_nv * 2 * _F * _phi_var[_qp]) + (_nyv * _fyv * _dop_var[_qp]) + 2 * _nvv * _fv * _u[_qp] + _R * _T * _nv * log(_u[_qp]/(1-_u[_qp])) + _lambv) * _test[_i][_qp];
}
...
{
  return -_cv * _grad_phi[_j][_qp] * _grad_test[_i][_qp] + 2 * _nvv * _fv * _phi[_j][_qp] * _test[_i][_qp] + _R * _T * _nv * ( 1 / (_u[_qp]*(1-_u[_qp]) ) ) * _test[_i][_qp] * _phi[_j][_qp]; 
}
/**/
