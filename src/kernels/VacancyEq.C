...
registerMooseObject("MooseApp", VacancyEq);

template <>
InputParameters
validParams<VacancyEq>()
{
...
VacancyEq::VacancyEq(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
VacancyEq::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _e_var)
  {
