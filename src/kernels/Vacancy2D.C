...
registerMooseObject("MooseApp", Vacancy2D);

template <>
InputParameters
validParams<Vacancy2D>()
{
  ...
  params.addParam<Real>("fyv", -4e4, "Electron-Vacancy interaction energy (fqv [J/mol]). Default is -4e4.");
...
Vacancy2D::Vacancy2D(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
Vacancy2D::computeQpResidual()
{
  return _gv * _grad_u[_qp] * _grad_test[_i][_qp] +
         ((_nv * 2 * _F * _phi_var[_qp]) + (_nqv * _fqv * _ele_var[_qp]) + (_nqv * _fyv * _Dop) +
	  2 * _nvv * _fv * _u[_qp] + _R * _T * _nv * log(_u[_qp] / (1 - _u[_qp])) + _lambv - /*_nv **/ _mu_var[_qp]) *
         _test[_i][_qp];
}

Real
Vacancy2D::computeQpJacobian()
{
...
Real
Vacancy2D::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _e_var)
  {
