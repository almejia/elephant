#include "Electron1D.h"

registerMooseObject("MooseApp", Electron1D);

template <>
InputParameters
validParams<Electron1D>()
{
...
Electron1D::Electron1D(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
Electron1D::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _v_var)
  {
