...
#include "ElectronEq.h"

registerMooseObject("MooseApp", ElectronEq);

template <>
InputParameters
validParams<ElectronEq>()
{
...
ElectronEq::ElectronEq(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
ElectronEq::computeQpResidual()
{
    return _gq * _grad_u[_qp] * _grad_test[_i][_qp] +
         ((-_nq * _F * _phi_var[_qp]) + (_nqv * _fqv * _vac_var[_qp]) +
          2 * _nqq * _fq * _u[_qp] + _R * _T * _nq * log(_u[_qp] / (1 - _u[_qp] - _Dop)) + _lambd) *
	  _test[_i][_qp];
}

Real
ElectronEq::computeQpJacobian()
{ 
...
Real
ElectronEq::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _v_var)
  {
