#include "EPotential1D.h"

registerMooseObject("MooseApp", EPotential1D);

template <>
InputParameters
validParams<EPotential1D>()
{
...
EPotential1D::EPotential1D(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
EPotential1D::computeQpResidual()
{
...
Real
EPotential1D::computeQpJacobian()
{
  return -(_eps0 * _epsr) * _grad_phi[_j][_qp] * _grad_test[_i][_qp];
}

Real
EPotential1D::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _e_var)
  {
