...
SurfacePCRes::computeQpJacobian()
{
  ...
  /*  return (-_grad_phi[_j][_qp] * _grad_test[_i][_qp] / (1-_u[_qp])) + (_grad_u[_qp] * _grad_u[_qp] * _phi[_j][_qp] / ((1-_u[_qp])*(1-_u[_qp]))) + (_eta1 * exp(-_dHdd1/(_R*_T))) * (-_Pco2 - (1/ (exp(_dS1/_R)*exp(-_dH1/(_R*_T))))) - (_eta2 * exp(-_dHdd2/(_R*_T))) * (_vac_var[_qp]*_ele_var[_qp]*_ele_var[_qp] + (_Pco * ( (1-_vac_var[_qp]) * (1-_ele_var[_qp]) * (1-_ele_var[_qp]) / (exp(_dS2/_R)*exp(-_dH2/(_R*_T))) ) ) ) * _phi[_j][_qp];
   */
  
  return (_Ns*_M * _R*_T)*(-_grad_phi[_j][_qp] * _grad_test[_i][_qp] / (1-_u[_qp])) + (_grad_u[_qp] * _grad_phi[_j][_qp] * _test[_i][_qp] / ((1-_u[_qp])*(1-_u[_qp]))) + (_eta1 * exp(-_dHdd1/(_R*_T))) * (-_Pco2 - (1/ (exp(_dS1/_R)*exp(-_dH1/(_R*_T))))) * _test[_i][_qp]*_phi[_j][_qp] - (_eta2 * exp(-_dHdd2/(_R*_T))) * (_vac_var[_qp]*_ele_var[_qp]*_ele_var[_qp] + (_Pco * ( (1-_vac_var[_qp]) * (1-_ele_var[_qp]) * (1-_ele_var[_qp]) / (exp(_dS2/_R)*exp(-_dH2/(_R*_T))) ) ))  * _test[_i][_qp]*_phi[_j][_qp];
}
