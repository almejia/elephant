...
validParams<EDiffusion>()
{
  ...
  params.addParam<Real>("ny", 41488.0, "Cation site density (ny). Default is 41488 = 4/(avnum*latpar^3).");
...
EDiffusion::EDiffusion(const InputParameters & parameters) : Kernel(parameters),
...
{
  //  std::cout << "\nqp: " << _qp << "\ti: " << _i << "\nphi: " << _grad_u[_qp] << "\ngradient: " << _grad_test[_i][_qp] << "\n u: " << _dop_var[_qp] << "\n v: " << _vac_var[_qp] << "\n test: " << _test[_i][_qp] << "\n";
  //  _x = 0;
  //  _x = (_eps0 * _epsr) * _grad_u[_qp] * _grad_test[_i][_qp] + (_F * ( 2 * _nv * _vac_var[_qp] - _ny * _Z *_dop_var[_qp] ) ) * _test[_i][_qp];
  //  std::cout << "\nphi: " << _u[_qp]; //(_eps0 * _epsr) * _grad_u[_qp] * _grad_test[_i][_qp] + (_F * ( 2 * _nv * _vac_var[_qp] - _ny * _Z *_dop_var[_qp] ) ) * _test[_i][_qp];
  return (_eps0 * _epsr) * _grad_u[_qp] * _grad_test[_i][_qp] + (_F * ( 2 * _nv * _vac_var[_qp] - _ny * _Z *_dop_var[_qp] ) ) * _test[_i][_qp];
}
